...
function MapaBrasilInterativo() {
    ...
    const nomeEstadoSelecionado = estadoSelecionado
        ? nomesEstados[estadoSelecionado]
        : null;

    const destinosFiltrados = nomeEstadoSelecionado
        ? destinos.filter(
            (destino) =>
                normalizarTexto(destino.estado) ===
                normalizarTexto(nomeEstadoSelecionado)
        )
        : [];
        ...
        <section className="mx-auto max-w-7xl px-6 py-20">
            <div className="mb-10 max-w-3xl">
                ...
                <p className="mt-4 text-lg leading-relaxed text-slate-500">
                    Clique em uma região ou em um estado do mapa para visualizar os pontos
                    ...
                    >
                        {Brazil.locations.map((location) => {
                            ...
                            if (regiaoSelecionada && regiao !== regiaoSelecionada) {
                                classe += ' estado-desativado';
                            }

                            if (estadoSelecionado === sigla) {
                                classe += ' estado-selecionado';
                            }
                ...
                <aside className="flex h-[620px] flex-col overflow-hidden rounded-[2rem] border border-slate-100 bg-white p-8 shadow-sm">
                    <h3 className="mb-4 text-3xl font-black text-[#485C11]">
                        {estadoSelecionado
                            ...
                            : regiaoSelecionada
                                ? nomesRegioes[regiaoSelecionada]
                                : 'Escolha uma região'}
                    </h3>
                    ...
                    {!regiaoSelecionada && !estadoSelecionado && (
                        <>
                            ...
                            Você pode clicar diretamente em um estado do mapa ou selecionar
                            uma região pelos botões abaixo.
                            </p>

                            <div className="flex flex-col gap-4">
                            {Object.entries(nomesRegioes).map(([id, nome]) => (
                                <button
                                key={id}
                                ...
                                <span
                                    ...
                                    style={{ backgroundColor: coresRegioes[id] }}
                                ></span>
                        ...
                        <div className="flex min-h-0 flex-1 flex-col">
                            <button
                            className="mb-6 w-fit rounded-full bg-[#DFECC6] px-4 py-3 font-black text-[#485C11] transition hover:bg-[#cfe4aa]"
                            ...
                            Agora escolha um estado da região{' '}
                            <strong className="text-[#485C11]">
                                {nomesRegioes[regiaoSelecionada]}
                            </strong>
                            .
                            </p>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <strong className="mb-1 block text-xl text-[#549E9C]">
                                    {sigla}
                                    </strong>
                    ...
                    {estadoSelecionado && (
                        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pr-2">
                            <button
                                className="w-fit shrink-0 rounded-full bg-[#DFECC6] px-4 py-3 font-black text-[#485C11] transition hover:bg-[#cfe4aa]"
                                onClick={limparSelecao}
                            >
                            ...
                            {destinosFiltrados.length > 0 ? (
                                destinosFiltrados.map((destino) => (
                                    <div
                                        key={destino.id}
                                        className="shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-slate-50"
                                    >
                                        <img
                                            src={destino.imagem}
                                            alt={destino.nome}
                                        ...
                                        <div className="p-4">
                                            <h4 className="mb-2 font-black text-[#485C11]">
                                                {destino.nome}
                                            </h4>
                                ...
                                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                                    <p className="m-0 text-slate-500">
                                        Ainda não existem pontos turísticos cadastrados para esse
                                        estado.
                                    </p>
                                </div>
                            )}
